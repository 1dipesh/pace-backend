...
from app.schemas.training_sync import Mutation
from app.services.training_sync_service import lock, bootstrap, envelope, project

router=APIRouter(prefix='/api/v1/sync/training',tags=['sync'],dependencies=[Depends(no_cache)])
...
def write(payload:Mutation,db:Session=Depends(get_db),user:PaceUser=Depends(get_current_user)):
    lock(db,user.id)
    fingerprint=hashlib.sha256(('training:'+json.dumps(payload.model_dump(mode='json'),sort_keys=True)).encode()).hexdigest()
    ...
    if conflicts: raise HTTPException(409,detail={'code':'training_conflict','current':conflicts})
    ...
    for (entity,cid),data in final.items():
        ...
        if entity=='templates' and any(not final.get(('exercises',i['exerciseId'])) for i in data['items']):
            raise HTTPException(422,detail={'code':'missing_exercise','id':cid})
        if entity=='workouts':
            for bundle in data['exercises']:
                for kind,item in [('log',bundle['log']),*[('set',s) for s in bundle['sets']]]:
                    key=(kind,item['id'])
                    if key in child_ids: raise HTTPException(422,detail={'code':'duplicate_workout_child','id':cid})
                    child_ids.add(key)
    changed=[]
    for c in sorted(payload.changes,key=lambda c: {'exercises':0,'settings':1,'templates':2,'activities':3,'hybrid':4,'workouts':5}[c.entity]):
